...
<html>
	...
	if(cookie_all!=null){
		for (int i = 0; i < cookie_all.length; i++) {
			String cName = cookie_all[i].getName();
			String cValue = cookie_all[i].getValue();
			if(cookie_all[i].getName()==null){ //없을경우
	 			...
	 			<ul>
					...
					if(cookie_all!=null){
						for (int i = 0; i < cookie_all.length; i++) {
							String cName = cookie_all[i].getName();
							String cValue = cookie_all[i].getValue();
							if(cookie_all[i].getName().equals("cookie_flag") && cValue.equals("success")){ //없을경우
									count=1;
					%>
